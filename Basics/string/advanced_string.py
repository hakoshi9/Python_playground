...
# 文字列は「+」を使用しなくとも、隣接しておけばPythonが結合してくれる
print('+演算子がなくても連結させることができる:', 'Py''thon')



"""文字列のインデックス"""
# 文字列はリストと同じような動きをするため、インデックス番号から1文字を取り出すこともできる
print('str1のインデックス0を出力:', str1[0])



"""リスト/タプルを文字列へ連結"""
# リストやタプルをjoin()メソッドを使って文字列に変換する
...
"""文字列の一部を置換"""
# replace()メソッドにより特定の文字列を別の特定文字列に置換する
...
"""文字列のスライス"""
# 文字列はリストと同じような動きをするため、リストと同じくスライスにより特定のインデックスから特定のインデックスまでを取り出すことができる
str7 = "スライス用文字列str7"
